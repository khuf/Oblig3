...
public class RobotGame extends JFrame {
	...
	private void createMainMenuPanel() {
		...
		btnStartSinglePlayer.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				gm.getSinglePlayerGame().getGameState().getPlayerA().setName(textFieldPlayerName.getText());
			}
		});
	...
	private void createSinglePlayerPanel() {
		...
		gm.getSinglePlayerGame().getGameState().getPlayerA().nameProperty().addListener((observable, oldValue, newValue) -> {
		...
		JLabel lblEnemyPlayerSP = new JLabel(gm.getSinglePlayerGame().getGameState().getPlayerB().getName());
		lblEnemyPlayerSP.setFont(new Font("Lucida Grande", Font.PLAIN, 24));
		gm.getSinglePlayerGame().getGameState().getPlayerB().nameProperty().addListener((observable, oldValue, newValue) -> {
		...
		gm.getSinglePlayerGame().getGameState().currentPositionProperty().addListener((observable, oldValue, newValue) -> {
		...
		btnAttackSP.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
				     ...
				     GameState state = gm.getSinglePlayerGame().getGameState();
		...
		gm.getSinglePlayerGame().getGameState().getPlayerA().energyProperty().addListener((observable, oldValue, newValue) -> {
		...
		gm.getSinglePlayerGame().getGameState().getPlayerB().energyProperty().addListener((observable, oldValue, newValue) -> {
	...
	private void createMultiPlayerPanel() {
		...
		listAvailableGamesMP.setModel(gm.getOpenGames());
		panelAvailableGamesMP.add(new JScrollPane(listAvailableGamesMP));
		...
		btnJoinGameMP.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (listAvailableGamesMP.isSelectionEmpty()) {
					 JOptionPane.showMessageDialog(new JFrame(), "Please select a game", "No game selected", 
								JOptionPane.INFORMATION_MESSAGE);
				}
				else {
					OnlinePlayer p = (OnlinePlayer) listAvailableGamesMP.getSelectedValue();
					System.out.println("Player chosen " + p.getName());
					gm.joinOnlineGame(textFieldPlayerName.getText(), p.getPlayerId());
				}
				/*CONTAINS LOGIC FOR JOINING A GAME
				 * 1. Get the selected player/playerid for the JList.
				 * 2. If the selected item is not empty, then proceed to join the game...
				 * 3. SQLManager.joinGame(playerId)
				 */
			}
		});
		...
		btnHostGameMP.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				gm.hostGame(textFieldPlayerName.getText());
				/*CONTAINS LOGIC FOR HOSTING A GAME
				 * 1. Create a new open game (new entry in open_games table).
				 * 2. Set status message to: "Waiting for opponent to join the game (label?)
				 * 3. If a player joins the game within 2 minutes, then delete current entry
				 * from 'open_games' and create a new entry within games_in_progress
				 * 4. Enable the attack button and disable it when the user have made his move
				 * !!!!!!!!SOME OF THIS LOGIC BELONGS IN MultiPlayerGame!!!!!!!!!
				 */
			}
		});
